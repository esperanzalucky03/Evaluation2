...
export default function TeacherAccountSettings() {
  ...
  useEffect(() => {
    ...
    const savedDept = localStorage.getItem("teacherDepartment")
    const saved2FA = localStorage.getItem("teacher2FA")

    setEmail(savedEmail || "")
    setFullName(savedName || "Teacher User")
    setPhone(savedPhone || "")
    setDepartment(savedDept || "English")
  ...
  const loadActivityLogs = () => {
    const logs = Object.keys(localStorage)
      .filter((key) => key.startsWith("teacher-activity-"))
      .map((key) => {
        try {
  ...
  const logActivity = (action: string) => {
    ...
    localStorage.setItem(`teacher-activity-${Date.now()}`, JSON.stringify(log))
  }
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="mt-2 w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500"
                  >
                ...
                <Button onClick={saveAccountInfo} className="bg-amber-600 hover:bg-amber-700 text-white">
                  Save Changes
                </Button>
              </CardContent>
            </Card>
            ...
            <>
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button onClick={handlePasswordChange} className="bg-amber-600 hover:bg-amber-700 text-white">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      onClick={() => {
                        ...
                        localStorage.setItem("teacher2FA", JSON.stringify(!twoFactorEnabled))
                        logActivity(`${!twoFactorEnabled ? "Enabled" : "Disabled"} 2FA`)
                        toast.success(`2FA ${!twoFactorEnabled ? "enabled" : "disabled"}`)
                      }}
                      className={twoFactorEnabled ? "bg-red-600 hover:bg-red-700" : "bg-amber-600 hover:bg-amber-700"}
                    >
